...
#define GB_BYTES (1024.0 * 1024.0 * 1024.0)
...
// Return memory used in GB with 2 decimal places
double get_mem_used(void)
{
        ...
        get_mem_info("Cached:", &cached) == 0)
    {
        ...
        double used_gb = used_bytes / GB_BYTES;
        return ((int)(used_gb * 100 + 0.5)) / 100.0; // Round to 2 decimal places
    }
    return -1.0;
}

// Return memory available in GB with 2 decimal places
double get_mem_available(void)
{
    ...
    {
        double available_gb = available / GB_BYTES;
        return ((int)(available_gb * 100 + 0.5)) / 100.0; // Round to 2 decimal places
    }
        ...
        get_mem_info("Cached:", &cached) == 0)
    {
        ...
        double available_gb = available_bytes / GB_BYTES;
        return ((int)(available_gb * 100 + 0.5)) / 100.0; // Round to 2 decimal places
    }
    return -1.0;
}

// Return total memory capacity in GB with 2 decimal places
double get_mem_total(void)
{
    long total = 0;
    if (get_mem_info("MemTotal:", &total) == 0)
    {
        double total_gb = total / GB_BYTES;
        return ((int)(total_gb * 100 + 0.5)) / 100.0; // Round to 2 decimal places
    }
    return -1.0;
}
...
// Return storage used in GB with 2 decimal places
double get_storage_used(void)
{
...
// Return storage available in GB with 2 decimal places
double get_storage_available(void)
{
    long long total = 0, free = 0;
    if (get_storage_info("/", &total, &free) == 0)
    {
        double available_gb = free / GB_BYTES;
        return ((int)(available_gb * 100 + 0.5)) / 100.0; // Round to 2 decimal places
    }
    return -1.0;
}

// Return total storage capacity in GB with 2 decimal places
double get_storage_total(void)
{
    long long total = 0, free = 0;
    if (get_storage_info("/", &total, &free) == 0)
    {
                       ...
                       netsnmp_request_info *requests)
{
    ...
    case MODE_GET:
    {
        ...
        case 9:
    {
        double mem_used = get_mem_used();
        if (mem_used != -1.0) {
            char value_str[32];
            snprintf(value_str, sizeof(value_str), "%.2f", mem_used);
            snmp_set_var_typed_value(var, ASN_OCTET_STR, (u_char*)value_str, strlen(value_str));
        } else {
            netsnmp_set_request_error(reqinfo, requests, SNMP_NOSUCHINSTANCE);
        }
...
case 10:
    {
        double mem_available = get_mem_available();
        if (mem_available != -1.0) {
            char value_str[32];
            snprintf(value_str, sizeof(value_str), "%.2f", mem_available);
            snmp_set_var_typed_value(var, ASN_OCTET_STR, (u_char*)value_str, strlen(value_str));
        } else {
            netsnmp_set_request_error(reqinfo, requests, SNMP_NOSUCHINSTANCE);
        }
...
case 11:
    {
        double storage_used = get_storage_used();
        if (storage_used != -1.0) {
            char value_str[32];
            snprintf(value_str, sizeof(value_str), "%.2f", storage_used);
            snmp_set_var_typed_value(var, ASN_OCTET_STR, (u_char*)value_str, strlen(value_str));
        } else {
            netsnmp_set_request_error(reqinfo, requests, SNMP_NOSUCHINSTANCE);
        }
...
case 12:
    {
        double storage_available = get_storage_available();
        if (storage_available != -1.0) {
            char value_str[32];
            snprintf(value_str, sizeof(value_str), "%.2f", storage_available);
            snmp_set_var_typed_value(var, ASN_OCTET_STR, (u_char*)value_str, strlen(value_str));
        } else {
            netsnmp_set_request_error(reqinfo, requests, SNMP_NOSUCHINSTANCE);
        }
    ...
    break;
        case 13:
            ...
            case MODE_SET_ACTION:
            {
                ...
                if (len >= sizeof(custom_timezone))
                    len = sizeof(custom_timezone) - 1;
